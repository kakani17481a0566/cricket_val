      ...
      <div className="space-y-10">
        {VALENTINE_WEEK.map((day, index) => {
          ...
          return (
            <div 
              key={day.id} 
              className={`relative flex flex-col md:flex-row items-center gap-6 p-8 rounded-[2rem] border-2 transition-all duration-500 hover:scale-[1.02] shadow-sm group
                ${active ? 'border-pink-400 bg-white ring-4 ring-pink-100/50 shadow-xl scale-[1.03]' : `${day.color} border-transparent hover:bg-white`}
              `}
            >
              {active && (
              ...
              <div className={`text-6xl md:text-7xl transform transition-transform group-hover:rotate-12 duration-300 ${active ? 'animate-heartbeat' : ''}`}>
                {day.icon}
              </div>
